...
Zakończonej produkcji nie można edytować zwykłym zapisem modelu ani formularzem. `ProductionSettlementReversalWorkflow` jest jawną, atomową procedurą cofnięcia: wymaga przyczyny, blokuje produkcję, usuwa automatyczne podanie, wycofuje FIFO, projekcję kosztową i partię, cofa status do etapu 1, odbudowuje FIFO od daty zmienionej produkcji oraz zapisuje audyt. Wcześniejsze dostawy, ruchy i produkcje pozostają bez zmian; późniejsze koszty partii oraz istniejących podań są synchronizowane z nowym wynikiem FIFO. Usunięcie przechodzi przez `delete_production_with_inventory` i korzysta z tego samego procesu. Relacje księgowe używają `RESTRICT`, więc zbiorcze usunięcie omijające akcję domenową nie skasuje zależności kaskadowo. Jeśli partię wykorzystało inne podanie, cofnięcie i zwykłe usunięcie są blokowane i wymagają osobnej korekty.
