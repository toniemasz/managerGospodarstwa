...
from feed.forms import FeedServingForm, PurchasedReadyFeedProductForm, ReadyFeedDeliveryForm
from feed.models import FeedProductModel, FeedServingModel
...
def create_ready_feed_product_view(request):
    farm = get_current_farm(request)
    form = PurchasedReadyFeedProductForm(request.POST or None, farm=farm)
    if request.method == "POST" and form.is_valid():
        try:
            product = create_purchased_ready_feed_product(farm=farm, **form.cleaned_data)
        except ValidationError as error:
            form.add_error("name", error.messages[0])
        else:
            log_action(farm=farm, user=request.user, action="READY_FEED_PRODUCT_CREATE", obj=product)
            messages.success(
                request,
                f"Utworzono gotową paszę „{product.name}”. Możesz teraz przyjąć jej pierwszą dostawę.",
            )
            return redirect("finished_feed_inventory")
    return render(request, "feed/ready_feed_product_form.html", {"form": form})


@login_required
@require_http_methods(["GET", "POST"])
def add_ready_feed_delivery_view(request, product_id):
    farm = get_current_farm(request)
    ...
    form = ReadyFeedDeliveryForm(request.POST or None, initial={"date": timezone.localdate()})
    if request.method == "POST" and form.is_valid():
        try:
            ...
            log_action(farm=farm, user=request.user, action="READY_FEED_DELIVERY_CREATE", obj=delivery)
            delivery_value = f"{delivery.total_cost:.2f}".replace(".", ",")
            messages.success(
                request,
                f"Przyjęto dostawę {format_mass(delivery.quantity_kg)} paszy „{product.name}” "
                f"o wartości {delivery_value} zł.",
            )
            return redirect("finished_feed_inventory")
    stock_kg = product.batches.aggregate(total=Sum("remaining_quantity_kg"))["total"] or 0
    return render(
        request,
        "feed/ready_feed_delivery_form.html",
        {"form": form, "product": product, "stock_kg": stock_kg},
    )
