...
def format_feed_cost_descriptions(apps, schema_editor):
    ...
    for cost in costs.iterator():
        ...
        if abs(quantity_kg) >= Decimal("1000"):
            quantity = quantity_kg / Decimal("1000")
            mass = f"{_number(quantity, 5)} t"
        else:
...
class Migration(migrations.Migration):
    dependencies = [("costs", "0002_production_costs")]
    operations = [
        migrations.RunPython(format_feed_cost_descriptions, migrations.RunPython.noop),
    ]
